...
import (
	...
	impl "github.com/mattmoor/korpc-sample/pkg/methods/one/magic"
)

type server struct {
  pb.UnimplementedOneServer
}


func (s *server) Magic(ctx context.Context, req *pb.Request) (*pb.Response, error) {
...
func main() {
	...
	pb.RegisterOneServer(grpcServer, &server{})
...
func probe() {
	...
	resp, err := healthpb.NewHealthClient(conn).Check(ctx, &healthpb.HealthCheckRequest{Service: "One"})
	if err != nil {
