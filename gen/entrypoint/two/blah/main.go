...
import (
	...
	impl "github.com/mattmoor/korpc-sample/pkg/methods/two/blah"
)

type server struct {
  pb.UnimplementedTwoServer
}


func (s *server) Blah(ctx context.Context, req *pb.Request) (*pb.Response, error) {
...
func main() {
	...
	pb.RegisterTwoServer(grpcServer, &server{})
...
func probe() {
	...
	resp, err := healthpb.NewHealthClient(conn).Check(ctx, &healthpb.HealthCheckRequest{Service: "Two"})
	if err != nil {
